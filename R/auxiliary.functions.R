...
#' @return The geometric mean, with some error-protection bits.
...
#' @param x A vector of sequencing counts from one observation
#' @return A vector of the same size as \code{x} after the centered log-ratio transformation
...
#' @param mat A sample by species count matrix
#' @param base The base of logarithm transformation. Default to be $e$.
#' @param const A small constant added to adjust negative values after centered log-ratio transformation. See details.
#' @return A data matrix of the same size as \code{mat} after the modified centered log-ratio transformation
