...
This document introduces to you how to use the package **metaMINT** for joint estimation of metagenomic/microbiome and metabolomic interaction network.
...
Alternatively, if the interest is to learn the conditional association (as defined by partial correlations) among bacterial taxa, one can use the function `cggm.precision`, which estimates the inverse covariance (or precision) matrix. 

```{r invcor}
est_InvCor <- cggm.precision(t(OTUs_mclr))
```

The model underlying **metaMINT** allows for both continuous and zero-inflated variables. Therefore, to jointly estimate the microbial and metabolomic interaction network, one only needs to expand the input to include both types of measurements. 

```{r MINT,eval=FALSE}
dat_combined <- t(rbind(OTUs_mclr,metabs))
est_InvCor_combined <- cggm.precision(dat_combined)
```
